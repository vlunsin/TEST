...
def save_to_excel(all_data, filename):
    ...
    # Write the data for each currency in individual sheets
    for date, exchange_data in all_data.items():
        for currency, exchange_rate in exchange_data.items():
            # Check if the sheet for the currency already exists
            if currency not in workbook.sheetnames:
                # Create a new sheet for each currency
                sheet = workbook.create_sheet(currency)
                # Write the headers (Date, Exchange Rate)
                sheet['A1'] = 'Date'
                sheet['B1'] = 'Exchange Rate'
            
            # Find the sheet corresponding to the currency
            sheet = workbook[currency]
            # Find the next available row to insert data
            next_row = len(sheet['A']) + 1
            # Insert the date and exchange rate into the respective columns
            sheet[f'A{next_row}'] = date
            sheet[f'B{next_row}'] = exchange_rate
